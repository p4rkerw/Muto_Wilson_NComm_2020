...
# read in bulk eset
bulk_eset <- readRDS("comparison_human_datasets/tcga_kidney_nontumor/eset.tcga.nontumor.bulkRna.rds")
...
p1 <- ggplot(toplot, aes(x=group, y=PT_KIM1, fill=group)) + 
  ...
  ggtitle("Proportion of PT_VCAM1 Cells by Patient Age Range TCGA-KIRC, KIRP, KICH")
...
p3 <- ggplot(toplot, aes(x=variable, y=value, fill=variable)) + 
  ...
  ggtitle("Proportion of Cells TCGA-KIRC, KIRP, KICH")

pdf("tcga_deconvolution.pdf")
CombinePlots(list(p1,p2))
p3
dev.off()
