...
#quote = "" is needed to read all the file content from PairsLigRec.txt
...
#find genes expressed in designated cell types
...
sub.markers <- FindAllMarkers(object = sub, logfc.threshold = 0, min.pct = 0.25, only.pos = TRUE)
marker.c1 <- filter(sub.markers, cluster == clusterID1) 
rownames(marker.c1) <- marker.c1$gene
marker.c2 <- filter(sub.markers, cluster == clusterID2)
rownames(marker.c2) <- marker.c2$gene
...
df <- cbind(ligRecPair,ligFC=2^(ligands[ligRecPair$Ligands,]$avg_logFC), recFC=2^(receptors[ligRecPair$Receptors,]$avg_logFC))
