#!/bin/bash
# this script will take a vcf and a cellranger bam file of paired-end reads (eg. libraries 4-5)
...
# bash healthy_dev/gatk/rna_allele_specific/step5_wasp_pe.sh Control_${SAMPLE} joint premrna
# done
...
STAR --genomeDir $SCRATCH1/starindex \
 --runThreadN 20 \
 --readFilesIn <(gunzip -c $WORK_DIR/$SAMPLE_NAME.filter.remap.fq1.gz) <(gunzip -c $WORK_DIR/$SAMPLE_NAME.filter.remap.fq2.gz)\
...
# python $WASP/mapping/rmdup_pe.py $WORK_DIR/$SAMPLE_NAME.keep.merge.sort.bam $OUTPUT_DIR/$SAMPLE_NAME.dedup.filter.wasp.bam
