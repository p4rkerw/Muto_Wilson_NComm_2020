...
DimPlot(rnaAggr, reduction = "umap", assay = "SCT", label = TRUE)

# visualize the clustering
# DimPlot(rnaAggr, reduction = "UMAP", assay = "SCT")
p1 <- DimPlot(rnaAggr, reduction = "umap", assay = "SCT", label = TRUE,group.by = "seurat_clusters") + ggtitle("snRNA Seurat Clustering with Harmony No Doublets")+NoLegend()
...
celltype.markers <- c("CUBN","HAVCR1","SLC5A1","SLC5A2", # PT and PT-KIM1+ markers
                      ...
                      "CALD1", # FIB
                      "PTPRC") # WBC
p2 <- DotPlot(rnaAggr, features = celltype.markers,cols = c("lightyellow","royalblue")) + RotatedAxis() + theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
new.cluster.ids <- c("PT","DCT1","PT","CNT","TAL","TAL","TAL","PC","IC-A","PEC","EC","IC-A","DR-PT","DCT2-CNT","POD","FIB-MES","IC-B","EC","EC","LEUK")
...
levels(rnaAggr) <- c("POD","PT","DR-PT","PEC","TAL","DCT1","DCT2-CNT","CNT","PC","IC-A","IC-B","EC","FIB-MES","LEUK")
...
lowres.cluster.ids <- c("POD","PT","PT","PEC","TAL","DCT_CNT_PC","DCT_CNT_PC","DCT_CNT_PC","DCT_CNT_PC",
                        "IC-A","IC-B","EC","FIB-MES","LEUK")
...
DimPlot(rnaAggr,label = T,reduction = "umap")+NoLegend()
...
p3 <- DimPlot(rnaAggr, reduction = "umap", assay = "SCT", label = TRUE) + ggtitle("snRNA Annotated Celltypes")+NoLegend()

features <- c("NPHS1","CUBN","HAVCR1","CFH","SLC12A1","SLC12A3","SLC8A1","AQP2","SLC26A7","SLC26A4","EMCN","ACTA2","PTPRC")
levels(rnaAggr) <- rev(c("POD","PT","DR-PT","PEC","TAL","DCT1","DCT2-CNT","CNT","PC","IC-A","IC-B","EC","FIB-MES","LEUK"))
p4 <- DotPlot(rnaAggr, features = rev(features),cols = c("lightyellow","royalblue")) + RotatedAxis() + theme(axis.text.x = element_text(angle = 90, hjust = 1))
levels(rnaAggr) <- c("POD","PT","DR-PT","PEC","TAL","DCT1","DCT2-CNT","CNT","PC","IC-A","IC-B","EC","FIB-MES","LEUK")


# draw pdf plots for before and after annotation
dir.create("plots", showWarnings = FALSE,width )
pdf(here("plots","1_umap.rnaAggr.pdf"),width =8, height=8)
p1+NoLegend()
dev.off()

pdf(here("plots","2_marker.rnaAggr.pdf"),width =8, height=6)
p2+RotatedAxis()
dev.off()

pdf(here("plots","3_celltype.rnaAggr.pdf"),width =8, height=8)
p3+NoLegend()
dev.off()

pdf(here("plots","4_celltype_marker.rnaAggr.pdf"),width =8, height=6)
p4+RotatedAxis()
dev.off()
